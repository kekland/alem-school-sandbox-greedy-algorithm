...
const newLocal = '../../headers'
const headers = require(newLocal)

const sendSolution = async (level) => {
  const response = await axios.post('https://cup.alem.school/api/sandbox/send_solution', {
    code: solutionCode,
    lang: 'js',
    level,
  }, { headers })
  ...
  return {
    level,
    status: gameResult.result[0].status,
